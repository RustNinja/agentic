...
3. Extend copied support-package item pruning deeper into monolithic protocol
   ...
   path prunes root and child-module items plus simple, transitive, and
   local-glob public facade reexports/imports when retained source names
   concrete dependency public symbols; remaining broad fallbacks are unresolved
   relative paths, unsupported glob prefixes, build-script packages,
