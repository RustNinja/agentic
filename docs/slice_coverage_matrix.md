...
| Analyzer modes | analyzer unit tests, CLI option parsing, Litter pinned corpus | The default CLI feature set enables `ra-hir` and defaults to bounded rust-analyzer HIR semantics for local workspace crates; files containing selected roots are analyzed first, exact project-local RA method/path targets are mapped into generic reduction hints and applied additively, while `syn` remains an explicit fallback |
