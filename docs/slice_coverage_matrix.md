...
| Fast macro/use corpus | `fast_macro_use.rs`, `fixtures/fast_macro_use`, `scripts/fast_fixture_loop.sh` | Tiny checked-in multi-package workspace for quick iteration across Litter-like renamed imports, local shadows, custom derives, serde-style helper attributes, UniFFI-shaped record/enum/object/export/constructor surfaces, async exported roots, retained callback/function-pointer signatures, platform `cfg_attr` modules, conversion impls, macro_rules, include-generated source, prelude/module reexports, inline child imports, type aliases, const/static items, enums, associated trait items, trait imports, and dead pruning |
