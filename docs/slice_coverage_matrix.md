...
| Source include assets | `manifest_hardening.rs` | Files referenced by retained `include!`, `include_str!`, and `include_bytes!` static literal/`concat!`/`CARGO_MANIFEST_DIR` paths are copied next to the sliced source; retained `include!` Rust source, unknown file include paths, absolute paths, external paths, and `OUT_DIR` generated assets are production-blocking |
| Compile-time environment macros | core production-readiness tests | Retained `env!`/`option_env!` macros that read outside Cargo manifest-derived package metadata are production-blocking because they can embed machine-local state |
...
| Non-test cfg roots and surfaces | core production-readiness tests, CLI production gate tests | Selected roots and retained source surfaces behind feature/platform cfgs, including `runtime-benchmarks`, are retained in the graph instead of being pruned as tests; selected cfg-gated roots remain production-blocking unless structured Cargo feature hints are covered by validation args, and non-root cfg surfaces carry structured package/module/file/cfg details and require compiler feedback for the selected matrix |
...
| Feature/platform cfg matrices | Only `#[cfg(test)]` is pruned as test-only; broader feature/platform matrix evaluation is conservative, cfg-gated selected roots are production-blocking unless concrete Cargo feature cfgs are covered by `--features` or `--all-features`, and retained non-root cfg surfaces require compiler feedback for the selected matrix |
