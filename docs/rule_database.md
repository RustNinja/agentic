...
| `fixture.litter.mobile_session_uniffi.001` | covered | Litter-shaped `codex-mobile-client` FFI session root slices through a UniFFI-style object/record API package, core session state, and protocol wire DTO package while pruning dead voice-handoff/settings modules, dead package modules, dead exported methods, and dead sibling DTO helpers |
...
| `trait.default_assoc_const_type_path.001` | covered | Trait associated const overrides stay with the trait item when selected code reads them through `Type::CONST`, not only explicit `<Type as Trait>::CONST` UFCS paths |
...
| `semantic.associated_external_call.no_method_cap.001` | covered | Unresolved associated calls such as `String::new()` and `std::collections::HashMap::new()` do not retain or cap local same-name methods unless the receiver prefix resolves to a local type |
| `semantic.method_fallback.typed_external_assoc.001` | covered | External/std associated calls are typed before fallback, so dependency constructors like `serde_json::Map::new()` do not retain unrelated local `new` methods |
...
| `semantic.method_fallback_cap.scoped_unknown.001` | covered | Capped unresolved method fallback hazards retain same-module methods named by the hazard as `blocked_by_unknown` without globally keeping dead same-named methods in unrelated modules |
...
| `fixture.uniffi_async_runtime_object_prune.support_chain.001` | covered | A checked-in support fixture slices through a UniFFI-shaped async exported object with `cfg_attr(..., uniffi::export(async_runtime = "tokio"))`, `derive(uniffi::Object/Record)`, an awaited method chain, and a shared `OnceLock<Arc<_>>` runtime dependency while pruning dead exported methods, dead render helpers, and dead runtime operations |
...
| `fixture.returned_dyn_trait_prune.support_chain.001` | covered | A checked-in support fixture slices through a returned `Box<dyn Trait>` and dynamic method call, keeps the required trait method and concrete impl, reports trait-object hazard evidence, and prunes dead dependency modules/functions plus unused trait default methods |
...
| `fixture.method_dispatch_prune.support_chain.001` | covered | A checked-in support fixture slices through string-typed method dispatch with a retained unknown fallback variant while pruning dead dispatch parameters, pruned variant payload structs/enums, same-name facade reexports, and now-empty support modules |
| `manifest.support_monolith_item_pruning.001` | covered | Monolithic protocol/support files prune dead same-file request/notification variants, payload structs/enums, impls, facade reexports, and empty child modules when those items are only reachable through pruned enum variants |
| `protocol.enum_method.compact.001` | covered | Enum parser/render methods like `from_wire` keep the selected enum, retained variants, and live payload methods while dropping payload-only items introduced by dead variants instead of treating their names as public surface |
| `match.enum_prune.unreachable_wildcard.001` | covered | After enum variant pruning, matches over the compacted enum drop now-unreachable wildcard/catch-all arms once all retained variants are already covered, keeping generated slices warning-clean under `--deny-warnings` |
...
| `static.global_mutex_registry.001` | covered | A checked-in support fixture slices through a multi-operation `OnceLock<Mutex<BTreeMap<_, Arc<_>>>>` registry, retaining register and lookup APIs while pruning remove/list/dead registry operations from the dependency crate |
...
| `pattern.external_protocol_variant_projection.001` | covered | External support protocol enum projections carry required variants through root usage, support facade reexports, and support-to-support type aliases while retaining only the payload methods called by live state reducers |
| `enum.public_support_internal_variant_prune.001` | covered | Public enums in support packages are narrowed to used or explicitly required variants when the enum is not protected by macro/serde/FFI-style contract attributes |
| `struct.public_support_internal_field_prune.001` | covered | Public structs in support packages narrow public fields to used fields unless the struct is selected or resolver-proven in a selected callable signature, preventing dead public support fields from surviving as accidental API surface |
| `struct.public_signature_field_surface.001` | covered | Public structs that are resolver-proven in selected callable signatures keep their public field surface, so support-field pruning does not break returned/input API contract types |
| `fixture.public_field_collision_prune.support_chain.001` | covered | A checked-in support fixture proves public support-field retention is concrete-struct scoped: a live `value` field on one dependency struct no longer keeps unrelated public `value`/`label` fields on another retained support struct, and cross-package struct literals are repaired after field pruning |
| `fixture.public_field_same_package_collision_prune.support_chain.001` | covered | A checked-in support fixture proves same-module support structs with the same public field name are no longer retained by package-wide field-name fallback; each colliding field needs concrete owner evidence or same-struct impl evidence |
| `fixture.public_field_typed_param_prune.support_chain.001` | covered | A checked-in support fixture proves public support fields used through typed function parameters in another package are retained by concrete owner evidence, while unused sibling fields and dead typed-param helpers are still pruned |
| `fixture.public_field_typed_param_pattern_prune.support_chain.001` | covered | A checked-in support fixture proves public support fields used by typed function parameter destructuring are retained by concrete pattern evidence, while same-module same-name sibling fields still prune |
| `fixture.public_field_typed_closure_prune.support_chain.001` | covered | A checked-in support fixture proves public support fields used only through typed closure parameters in another package are retained by concrete owner evidence, while unused sibling fields and dead closure helpers are still pruned |
| `fixture.public_field_typed_closure_pattern_prune.support_chain.001` | covered | A checked-in support fixture proves public support fields used by typed closure parameter destructuring are retained by concrete pattern evidence, while unused sibling fields and dead closure helpers are still pruned |
| `macro.impl_signature_surface_collision.001` | covered | Macro-bearing inherent impl surfaces are retained for selected callable signature types only when the type path resolves to the actual signature item, preventing same-name support types from keeping dead exported impl methods |
...
| `fixture.iterator_flatten_option_array_prune.support_chain.001` | covered | A checked-in support fixture slices through array-backed `Option<T>` iterator `flatten().map(...)`, retaining only the live payload render method |
| `fixture.iterator_flatten_option_refs_prune.support_chain.001` | covered | A checked-in support fixture slices through Litter-shaped `[&Option<T>]::into_iter().flatten()` references while pruning dead support methods |
| `fixture.iterator_flatten_result_vec_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec<Result<T, E>>::into_iter().flatten().map(...)`, retaining OK payload methods without unused error methods |
| `fixture.iterator_flatten_result_refs_prune.support_chain.001` | covered | A checked-in support fixture slices through referenced `Result<T, E>` flattening and keeps only the OK payload render closure |
| `fixture.iterator_flatten_vec_vec_prune.support_chain.001` | covered | A checked-in support fixture slices through nested `Vec<Vec<T>>::into_iter().flatten()` payload flow without dead nested-vector support |
| `fixture.iterator_flatten_take_map_prune.support_chain.001` | covered | A checked-in support fixture combines `flatten().take(...).map(...)` so pass-through adapters do not widen the support closure |
| `fixture.iterator_filter_map_result_ok_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map(Result::ok)`, retaining OK payload methods while pruning unused error methods |
| `fixture.iterator_filter_map_result_err_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map(Result::err)`, retaining error render methods while pruning unused OK render methods |
| `fixture.iterator_find_map_result_ok_prune.support_chain.001` | covered | A checked-in support fixture slices through `find_map(Result::ok)` followed by OK payload mapping without dead support leakage |
| `fixture.iterator_filter_map_option_identity_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map(|payload| payload)` over `Option<T>` collections while pruning dead payload methods |
| `fixture.iterator_filter_map_match_result_ok_question_prune.support_chain.001` | covered | A checked-in support fixture slices through Litter-shaped `filter_map` match arms that call `Result::ok()?` before a payload method while pruning dead error methods |
| `fixture.iterator_filter_map_match_option_question_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map` match arms that use `Option` `?` helper lookups and keep only the selected payload render path |
| `fixture.iterator_find_map_method_ref_option_prune.support_chain.001` | covered | A checked-in support fixture slices through `find_map(Type::method)` over borrowed iterator items and retains the method-reference payload closure |
| `fixture.hashmap_values_find_map_method_ref_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::values().find_map(Type::method)` without retaining unused map payload methods |
| `fixture.btreemap_values_find_map_method_ref_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::values().find_map(Type::method)` without retaining unused map payload methods |
| `fixture.iterator_find_map_enum_match_prune.support_chain.001` | covered | A checked-in support fixture slices through enum payload matches inside `find_map` closures while pruning dead payload methods |
| `fixture.iterator_filter_map_borrowed_field_match_prune.support_chain.001` | covered | A checked-in support fixture slices through borrowed `Option` struct fields inside `filter_map` and prunes a private pure-initialized unused field from retained constructors |
| `fixture.iterator_filter_map_let_else_option_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map` closures that use `let Some(...) else` before a payload method |
| `fixture.iterator_flat_map_match_enum_vec_prune.support_chain.001` | covered | A checked-in support fixture slices through enum matches that expand to `Vec<T>` inside `flat_map` and then render the live payload |
| `fixture.iterator_find_map_nested_ok_question_prune.support_chain.001` | covered | A checked-in support fixture slices through nested `find_map` closures using `Result::ok()?` local bindings and prunes unused error methods |
| `fixture.hashmap_iter_filter_map_match_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::iter()` key/value tuple matches inside `filter_map` without retaining dead payload methods |
| `fixture.iterator_filter_map_method_ref_option_prune.support_chain.001` | covered | A checked-in support fixture slices through `filter_map(Type::method)` where the method returns `Option<String>` and keeps only the method-reference payload path |
| `fixture.iterator_for_loop_flatten_option_refs_prune.support_chain.001` | covered | A checked-in support fixture slices through a `for` loop over `[&Option<T>]::into_iter().flatten()` and keeps only the used loop payload method |
| `fixture.iterator_for_loop_flatten_result_vec_prune.support_chain.001` | covered | A checked-in support fixture slices through a `for` loop over `Vec<Result<T, E>>::into_iter().flatten()` without retaining unused error methods |
...
| `fixture.str_split_once_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::split_once(...).map(...)`, retaining only parser payload construction and render methods |
| `fixture.str_strip_prefix_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::strip_prefix(...).map(Type::new).map(...)`, retaining method-reference construction without dead parser helpers |
| `fixture.str_lines_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::lines().filter_map(...)`, retaining live line payload methods and pruning dead support siblings |
| `fixture.str_split_whitespace_find_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::split_whitespace().find_map(Result::ok().map(...))`, retaining fallible parser surfaces without dead error render helpers |
| `fixture.str_rsplit_once_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::rsplit_once(...).map(...)`, retaining trailing parser payload construction and render methods only |
| `fixture.str_splitn_find_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::splitn(...).map(Type::new).find_map(...)`, retaining optional parser payload methods without dead siblings |
| `fixture.str_split_terminator_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::split_terminator(...).filter_map(...)`, retaining non-empty segment render methods only |
| `fixture.str_char_indices_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::char_indices().filter_map(...)`, retaining formatted index/char payload construction without dead string helpers |
| `fixture.str_match_indices_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::match_indices(...).map(...)`, retaining match-position payload rendering only |
| `fixture.path_components_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::components().filter_map(...)`, retaining `Component::Normal` payload rendering without dead path helpers |
| `fixture.path_file_name_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::file_name().and_then(OsStr::to_str).map(...)`, retaining filename payload methods only |
| `fixture.osstr_to_str_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `OsStr::to_str().map(...)`, retaining UTF-8 OS-string payload rendering without dead helpers |
| `fixture.hashmap_get_key_value_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::get_key_value(...).map(...)`, retaining key/value payload rendering without dead map support |
| `fixture.btreemap_range_mut_find_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::range_mut(...).find_map(...)`, retaining mutable ranged value methods only |
| `fixture.iterator_peekable_peek_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Iterator::peekable().peek().map(...)`, retaining peeked borrowed payload methods without dead iterator helpers |
| `fixture.iter_repeat_with_take_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `std::iter::repeat_with(...).take(...).map(...)`, retaining iterator-factory payload construction and render methods only |
| `fixture.str_chars_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::chars().filter_map(...)`, retaining character payload construction and render methods only |
| `fixture.str_bytes_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::bytes().filter_map(...)`, retaining byte payload construction and render methods only |
| `fixture.str_split_inclusive_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::split_inclusive(...).filter_map(...)`, retaining non-empty segment payload methods only |
| `fixture.str_rsplitn_find_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::rsplitn(...).find_map(...)`, retaining reverse split payload rendering only |
| `fixture.str_trim_matches_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::trim_matches(...)` inside an iterator map, retaining trimmed payload rendering only |
| `fixture.str_strip_suffix_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::strip_suffix(...).map(...)`, retaining suffix-stripped payload rendering only |
| `fixture.path_parent_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::parent().and_then(Path::to_str).map(...)`, retaining parent path payload rendering only |
| `fixture.path_ancestors_find_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::ancestors().find_map(...)`, retaining file-name payload rendering only |
| `fixture.path_extension_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::extension().and_then(OsStr::to_str).map(...)`, retaining extension payload rendering only |
| `fixture.path_file_stem_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::file_stem().and_then(OsStr::to_str).map(...)`, retaining file-stem payload rendering only |
| `fixture.path_strip_prefix_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::strip_prefix(...).ok().and_then(Path::to_str).map(...)`, retaining stripped path payload rendering only |
| `fixture.hashmap_values_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::values().filter_map(...)`, retaining value payload rendering only |
| `fixture.hashmap_keys_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::keys().map(...)`, retaining key-derived payload rendering only |
| `fixture.btreemap_values_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::values().filter_map(...)`, retaining ordered-map value payload rendering only |
| `fixture.btreemap_keys_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::keys().map(...)`, retaining ordered key-derived payload rendering only |
| `fixture.btreeset_first_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeSet::first().map(...)`, retaining first-value payload rendering only |
| `fixture.btreeset_last_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeSet::last().map(...)`, retaining last-value payload rendering only |
| `fixture.binaryheap_peek_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BinaryHeap::peek_mut().map(...)`, retaining mutable top-value payload rendering only |
| `fixture.vecdeque_range_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `VecDeque::range(...).map(...)`, retaining ranged payload rendering only |
| `fixture.linkedlist_iter_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `LinkedList::iter_mut().map(...)`, retaining mutable payload methods and pruning read-only siblings |
| `fixture.str_split_ascii_whitespace_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::split_ascii_whitespace().map(...)`, retaining whitespace token payload rendering only |
| `fixture.str_rmatch_indices_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::rmatch_indices(...).map(...)`, retaining reverse match-position payload rendering only |
| `fixture.str_encode_utf16_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::encode_utf16().filter_map(...)`, retaining UTF-16 unit payload rendering only |
| `fixture.str_char_escape_default_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `char::escape_default` flat-map chains, retaining escaped character payload rendering only |
| `fixture.path_iter_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::iter().filter_map(OsStr::to_str).map(...)`, retaining path component payload rendering only |
| `fixture.path_with_file_name_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::with_file_name(...).to_str().map(...)`, retaining renamed path payload rendering only |
| `fixture.path_with_extension_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Path::with_extension(...).to_str().map(...)`, retaining extension-mutated path payload rendering only |
| `fixture.hashmap_into_keys_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::into_keys().map(...)`, retaining owned key payload rendering only |
| `fixture.btreemap_into_keys_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::into_keys().map(...)`, retaining owned ordered-key payload rendering only |
| `fixture.vec_first_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::first_mut().map(...)`, retaining mutable first-item payload methods only |
| `fixture.vec_last_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::last_mut().map(...)`, retaining mutable last-item payload methods only |
| `fixture.vec_get_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::get_mut(...).map(...)`, retaining mutable indexed payload methods only |
| `fixture.vecdeque_get_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `VecDeque::get(...).map(...)`, retaining indexed deque payload rendering only |
| `fixture.vecdeque_get_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `VecDeque::get_mut(...).map(...)`, retaining mutable indexed deque payload methods only |
| `fixture.vecdeque_remove_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `VecDeque::remove(...).map(...)`, retaining removed deque payload rendering only |
| `fixture.linkedlist_front_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `LinkedList::front_mut().map(...)`, retaining mutable front payload methods only |
| `fixture.linkedlist_back_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `LinkedList::back_mut().map(...)`, retaining mutable back payload methods only |
| `fixture.btreemap_first_entry_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::first_entry().map(...)`, retaining mutable first-entry payload methods only |
| `fixture.btreemap_last_entry_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::last_entry().map(...)`, retaining mutable last-entry payload methods only |
| `fixture.binaryheap_push_peek_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BinaryHeap::push(...); peek().map(...)`, retaining pushed top-value payload rendering only |
| `fixture.iterator_map_while_result_ok_prune.support_chain.001` | covered | A checked-in support fixture slices through `Iterator::map_while` over `Result::ok`, retaining fallible payload parsing and live render methods only |
| `fixture.iterator_scan_stateful_map_prune.support_chain.001` | covered | A checked-in support fixture slices through stateful `Iterator::scan` closures, retaining generated state-tag payload methods without dead support code |
| `fixture.iterator_flatten_option_result_prune.support_chain.001` | covered | A checked-in support fixture slices through nested `Option<Result<T, E>>` iterator flattening plus `Result::ok`, retaining only the live payload path |
| `fixture.option_zip_tuple_render_prune.support_chain.001` | covered | A checked-in support fixture slices through `Option::zip` tuple payload rendering, retaining both live tuple-side receiver methods and pruning dead siblings |
| `fixture.slice_windows_filter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `[T]::windows(...).filter_map(...)`, retaining window payload methods without broad slice helper retention |
...
| `fixture.vec_dedup_by_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::dedup_by` followed by live payload rendering without retaining unrelated vector helpers |
| `fixture.vec_sort_by_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::sort_by` comparator closures and keeps only rendered payload methods |
| `fixture.vec_select_nth_unstable_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::select_nth_unstable_by`, retaining comparator field access and the selected payload renderer only |
| `fixture.vec_split_at_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::split_at` tuple slice views and downstream payload rendering only |
| `fixture.vec_split_at_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::split_at_mut` mutable slice views and keeps only mutable payload methods |
| `fixture.vec_chunks_exact_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::chunks_exact` slice chunks and prunes dead chunk payload helpers |
| `fixture.vec_windows_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::windows` slice views and keeps only live window payload rendering |
| `fixture.vec_retain_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::retain` predicate field access and rendered payload closure only |
| `fixture.string_split_off_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::split_off`, retaining only split tail payload rendering |
| `fixture.string_truncate_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::truncate` mutation and keeps only the rendered string payload path |
| `fixture.string_clear_push_str_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::clear` plus `push_str`, retaining only live string payload rendering |
| `fixture.string_retain_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::retain` character predicates without retaining dead string helpers |
| `fixture.str_trim_start_matches_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::trim_start_matches` adapters and live payload construction only |
| `fixture.str_trim_end_matches_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::trim_end_matches` adapters and prunes dead sibling helpers |
| `fixture.str_split_inclusive_rev_map_prune.support_chain.001` | covered | A checked-in support fixture slices through reversed `str::split_inclusive` adapters and constructor method references |
| `fixture.str_rsplit_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::rsplit` segment iterators and keeps only live segment rendering |
| `fixture.pathbuf_join_file_name_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `PathBuf::join` plus `file_name` extraction without retaining dead path helpers |
...
| `fixture.vec_reserve_push_get_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::reserve` plus `push` and indexed payload rendering without retaining dead vector helpers |
| `fixture.vec_shrink_to_fit_last_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::shrink_to_fit` and last-item rendering while pruning unused siblings |
| `fixture.vec_truncate_get_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::truncate` followed by indexed live payload rendering only |
| `fixture.vec_split_off_last_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `Vec::split_off` tail ownership and live payload rendering without dead helpers |
| `fixture.slice_sort_by_key_first_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice `sort_by_key` closure method dependencies and first-item rendering only |
| `fixture.slice_rotate_left_first_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice `rotate_left` mutation and keeps only the live rotated payload renderer |
| `fixture.slice_rotate_right_last_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice `rotate_right` mutation and live last-item rendering only |
| `fixture.string_reserve_push_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::with_capacity`/`reserve` plus push operations and live payload rendering only |
| `fixture.string_shrink_to_fit_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `String::shrink_to_fit` after mutation while pruning unrelated string helpers |
| `fixture.str_as_bytes_first_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `str::as_bytes().first()` byte payload conversion without dead string helpers |
| `fixture.str_chars_next_back_map_prune.support_chain.001` | covered | A checked-in support fixture slices through reverse-end `str::chars().next_back()` payload conversion only |
| `fixture.pathbuf_push_pop_push_to_str_map_prune.support_chain.001` | covered | A checked-in support fixture slices through repeated `PathBuf::push`/`pop` mutation and live string conversion only |
| `fixture.osstr_to_string_lossy_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `OsStr::to_string_lossy` and live payload rendering without dead OS-string helpers |
| `fixture.cstr_to_str_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `CString::as_c_str().to_str()` conversion and live payload rendering only |
| `fixture.hashmap_shrink_to_fit_values_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::shrink_to_fit` plus value iteration and prunes dead map helpers |
| `fixture.hashmap_keys_cloned_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashMap::keys().cloned()` key payload rendering without retaining dead values helpers |
| `fixture.hashset_reserve_insert_iter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashSet::reserve` plus insert/iteration and live set payload rendering only |
| `fixture.hashset_extend_iter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `HashSet::extend` and live iterated payload rendering without dead set support |
| `fixture.btreemap_append_values_last_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::append` plus last value rendering and prunes dead ordered-map helpers |
| `fixture.btreemap_clear_insert_values_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeMap::clear` plus replacement insert/value rendering only |
| `fixture.btreeset_append_iter_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BTreeSet::append` and live iterated key rendering without retaining dead set helpers |
| `fixture.binaryheap_push_pop_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BinaryHeap::push` plus `pop` owned payload rendering only |
| `fixture.binaryheap_into_sorted_vec_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `BinaryHeap::into_sorted_vec` and downstream payload rendering only |
| `fixture.linkedlist_push_back_front_map_prune.support_chain.001` | covered | A checked-in support fixture slices through `LinkedList::push_back` plus front payload rendering without dead list helpers |
...
| `build.out_dir_expression_include.001` | covered | Build-script generated expression Rust such as `helper()` is scanned from string literals, so helpers referenced only by `include!(concat!(env!("OUT_DIR"), ...))` remain blocked-by-unknown while unrelated dead helpers prune |
| `build.out_dir_string_item_include.001` | covered | Root-level `include!(concat!(env!("OUT_DIR"), ...))` item source generated from a string literal is retained when selected code mentions a generated declaration, and only helper dependencies referenced by the generated source block pruning |
| `build.out_dir_concat_expression_include.001` | covered | Build-script generated expression Rust assembled with literal `concat!(...)` is evaluated before scanning, so helpers referenced only by fragmented generated source remain blocked-by-unknown while unrelated dead helpers prune |
| `build.out_dir_format_expression_include.001` | covered | Build-script generated expression Rust assembled through literal-only `format!`/`write!`/`writeln!` payloads is evaluated before scanning, so helpers split across format fragments remain blocked-by-unknown while unrelated dead helpers prune |
| `build.out_dir_quote_expression_include.001` | covered | Build-script generated expression Rust assembled through non-interpolated `quote!` token bodies is scanned before pruning, so helpers referenced only by quoted generated source remain blocked-by-unknown while unrelated dead helpers prune |
| `build.out_dir_quote_attribute_item_include.001` | covered | Non-interpolated quoted generated item source containing Rust attributes is scanned without mistaking `#[...]` for quote interpolation, so helper dependencies referenced by generated functions remain blocked while unrelated helpers prune |
...
| `request.json_patch_state_machine.001` | covered | A checked-in request/state support fixture proves JSON patch state machines with serde adjacent tag/content DTOs retain only the selected API, serde contract payloads, and live state reducer methods while pruning dead support modules and same-name constructor fallbacks |
...
| `manifest.patch_path_dependency_scoped.001` | covered | Workspace patch path packages are discovered dependency-by-dependency, so retained patched transitive dependencies are copied and emitted while unused patch entries do not create redundant support packages |
| `manifest.cdylib_staticlib_target_patch_bundle.001` | covered | FFI-shaped crates preserve `cdylib`/`staticlib` crate types, target-specific patched dependencies, feature entries, lockfiles, and toolchain context while pruning unrelated patch packages |
...
| `macro.proc_macro_helper_call_scope.001` | covered | Proc-macro helper traversal follows actual helper function calls instead of every matching identifier, so a local variable named like a dead helper does not retain dead generated support dependencies |
| `macro.proc_macro_module_helper_scope.001` | covered | Proc-macro expansion evidence follows module-qualified helper calls such as `builders::live_tokens()` through local proc-macro crate modules without falling back to same-name external or dead helper matches |
| `macro.proc_macro_helper_use_alias.001` | covered | Proc-macro expansion evidence follows local helper imports such as `use builders::live_tokens as render_live; render_live()` without retaining dead sibling builders |
...
| `fixture.slice_first_chunk_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice first_chunk map payload recovery while pruning dead support package items |
| `fixture.slice_first_chunk_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice first_chunk_mut map payload recovery while pruning dead support package items |
| `fixture.slice_last_chunk_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice last_chunk map payload recovery while pruning dead support package items |
| `fixture.slice_last_chunk_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice last_chunk_mut map payload recovery while pruning dead support package items |
| `fixture.slice_split_first_chunk_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_first_chunk map payload recovery while pruning dead support package items |
| `fixture.slice_split_first_chunk_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_first_chunk_mut map payload recovery while pruning dead support package items |
| `fixture.slice_split_last_chunk_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_last_chunk map payload recovery while pruning dead support package items |
| `fixture.slice_split_last_chunk_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_last_chunk_mut map payload recovery while pruning dead support package items |
| `fixture.slice_as_chunks_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice as_chunks iter payload recovery while pruning dead support package items |
| `fixture.slice_as_chunks_mut_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice as_chunks_mut iter payload recovery while pruning dead support package items |
| `fixture.slice_as_rchunks_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice as_rchunks iter payload recovery while pruning dead support package items |
| `fixture.slice_as_rchunks_mut_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice as_rchunks_mut iter payload recovery while pruning dead support package items |
| `fixture.slice_split_at_checked_tail_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_at_checked tail iter payload recovery while pruning dead support package items |
| `fixture.slice_split_at_mut_checked_tail_iter_prune.support_chain.001` | covered | A checked-in support fixture slices through slice split_at_mut_checked tail iter payload recovery while pruning dead support package items |
| `fixture.option_as_pin_ref_map_prune.support_chain.001` | covered | A checked-in support fixture slices through option as_pin_ref map payload recovery while pruning dead support package items |
| `fixture.option_as_pin_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through option as_pin_mut map payload recovery while pruning dead support package items |
| `fixture.vecdeque_range_mut_map_prune.support_chain.001` | covered | A checked-in support fixture slices through vecdeque range_mut map payload recovery while pruning dead support package items |
| `fixture.for_hashmap_iter_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for hashmap iter pair for-loop payload typing while pruning dead support package items |
| `fixture.for_hashmap_iter_mut_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for hashmap iter mut pair for-loop payload typing while pruning dead support package items |
| `fixture.for_hashmap_into_iter_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for hashmap into iter pair for-loop payload typing while pruning dead support package items |
| `fixture.for_hashmap_drain_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for hashmap drain pair for-loop payload typing while pruning dead support package items |
| `fixture.for_btreemap_iter_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreemap iter pair for-loop payload typing while pruning dead support package items |
| `fixture.for_btreemap_iter_mut_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreemap iter mut pair for-loop payload typing while pruning dead support package items |
| `fixture.for_btreemap_into_iter_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreemap into iter pair for-loop payload typing while pruning dead support package items |
| `fixture.for_btreemap_range_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreemap range pair for-loop payload typing while pruning dead support package items |
| `fixture.for_btreemap_range_mut_pair_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreemap range mut pair for-loop payload typing while pruning dead support package items |
| `fixture.for_vec_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for vec ref payload for-loop payload typing while pruning dead support package items |
| `fixture.for_vec_mut_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for vec mut payload for-loop payload typing while pruning dead support package items |
| `fixture.for_array_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for array ref payload for-loop payload typing while pruning dead support package items |
| `fixture.for_option_iter_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for option iter payload for-loop payload typing while pruning dead support package items |
| `fixture.for_option_iter_mut_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for option iter mut payload for-loop payload typing while pruning dead support package items |
| `fixture.for_result_iter_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for result iter payload for-loop payload typing while pruning dead support package items |
| `fixture.for_result_iter_mut_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for result iter mut payload for-loop payload typing while pruning dead support package items |
| `fixture.for_vecdeque_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for vecdeque ref payload for-loop payload typing while pruning dead support package items |
| `fixture.for_linkedlist_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for linkedlist ref payload for-loop payload typing while pruning dead support package items |
| `fixture.for_hashset_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for hashset ref payload for-loop payload typing while pruning dead support package items |
| `fixture.for_btreeset_ref_payload_prune.support_chain.001` | covered | A checked-in support fixture slices through for btreeset ref payload for-loop payload typing while pruning dead support package items |
| `manifest.support_path_dependency_internal_field_prune.001` | covered | A manifest hardening fixture proves restricted external path support packages prune unused pure helper struct fields and matching pure initializers instead of copying the whole support struct verbatim |
| `manifest.support_path_dependency_signature_field_surface.001` | covered | A manifest hardening fixture proves external path support structs directly exposed by the selected root signature keep their public field surface while unrelated support functions still prune |
| `manifest.support_path_dependency_facade_field_surface.001` | covered | Public facade/glob/module-alias support paths mark the resolved support struct as API surface so root field accesses do not lose required fields during internal support pruning |
| `manifest.impl_trait_surface_method_dependency.001` | covered | Retained trait impl methods feed self-method and associated item references back into inherent impl retention, so autoderef-style helpers such as `self.as_str()` remain while dead siblings prune |
| `manifest.impl_non_fn_import_precision.001` | covered | Imports mentioned only by pruned inherent associated const/type items are pruned instead of being kept by broad retained-impl token scans |
| `manifest.pub_crate_reexport_private_precision.001` | covered | Restricted `pub(crate)`/non-public reexports are treated like local imports for pruning unless retained code actually uses that visible alias |
| `dynamic.returned_trait_object_helper_construction.001` | covered | Returned trait-object surfaces are proven when the selected function forwards through a local helper whose returned value constructs a concrete local implementor; direct input-forwarded trait objects remain review-required |
