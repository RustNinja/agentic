...
| `enum.public_support_internal_variant_prune.001` | covered | Public enums in support packages are narrowed to used variants when the enum is not selected, not in a selected callable signature, and not protected by macro/serde/FFI-style contract attributes |
