...
| `manifest.support_dependency_item_pruning.001` | covered | No-build copied support packages with concrete retained public names prune dead sibling items in the library root and child modules, narrow public facade reexports, copy only live static include assets, and omit local path dependencies used only by dead support items |
