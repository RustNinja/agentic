...
fn rendered_item_surface_idents_for_mention_index(
    project: &Project,
    reduced: &ReducedProject,
    item: &ItemId,
    render_plan: Option<&RenderPlan>,
    ...
    match &record.item {
        ...
        _ => return rendered_item_surface_idents(project, reduced, item),
    }
    expand_alias_surface_idents(&record.aliases, &mut idents);
    idents
}
...
fn collect_struct_surface_idents_for_mention_index(
    ...
    match &item_struct.fields {
        ...
        _ => collect_token_idents(&item_struct.to_token_stream(), idents),
    }
}
...
fn trait_assoc_item_should_remain_for_references(
    project: &Project,
    reduced: &ReducedProject,
    package: &str,
    module_path: &[String],
    ...
    reachable_reduced_callables_reference_trait_assoc_item(
        project,
        reduced,
        package,
        &trait_path,
        assoc_name,
    ) || reachable_items_reference_trait_assoc_item(
        project,
        reduced,
        package,
        &trait_path,
...
fn reachable_reduced_callables_reference_trait_assoc_item(
    ...
    reduced.reachable.iter().any(|callable| {
        ...
        let self_trait = match callable {
            CallableId::Method {
                package,
                trait_path: Some(trait_path),
                ..
            } => Some((package.as_str(), trait_path.as_slice())),
            _ => None,
        };
        let mut visitor = TraitAssocPathReferenceVisitor::new(
            project,
...
fn reachable_items_reference_trait_assoc_item(
    project: &Project,
    reduced: &ReducedProject,
    target_package: &str,
    target_trait_path: &[String],
    ...
    reduced.reachable_items.iter().any(|item_id| {
        ...
        let self_trait = (item_id.package == target_package
            && item_id.kind == ItemKind::Trait
            && path_from_item(item_id) == target_trait_path)
            .then_some((target_package, target_trait_path));
        let mut visitor = TraitAssocPathReferenceVisitor::new(
            project,
            &record.package,
            &record.module_path,
...
fn reachable_items_macro_tokens_mention_ident(
    project: &Project,
    reduced: &ReducedProject,
    ident: &str,
) -> bool {
    reduced.reachable_items.iter().any(|item_id| {
        project.items.get(item_id).is_some_and(|record| {
...
struct TraitAssocPathReferenceVisitor<'a> {
    project: &'a Project,
    package: &'a str,
    module_path: &'a [String],
    aliases: &'a HashMap<String, Vec<String>>,
    ...
    assoc_name: &'a str,
    self_trait: Option<(&'a str, &'a [String])>,
    found: bool,
}

impl<'a> TraitAssocPathReferenceVisitor<'a> {
    #[allow(clippy::too_many_arguments)]
    fn new(
        project: &'a Project,
        package: &'a str,
    ...
    fn qself_path_references_target(&self, qself: &syn::QSelf, segments: &[String]) -> bool {
        ...
        if qself.position == 0 {
            return self.type_references_target_trait(&qself.ty);
        }
        let trait_segment_count = qself.position.min(segments.len().saturating_sub(1));
        self.resolve_trait_segments(&segments[..trait_segment_count])
            .is_some_and(|(package, path)| {
    ...
    fn segments_reference_target(&self, segments: &[String]) -> bool {
        ...
        if trait_segments.len() == 1 && trait_segments[0] == "Self" {
            return self.self_trait_is_target();
        }
        self.resolve_trait_segments(trait_segments)
            .is_some_and(|(package, path)| {
                package == self.target_package && path == self.target_trait_path
            })
    }

    fn path_has_target_assoc_binding(&self, path: &syn::Path) -> bool {
        let Some(last) = path.segments.last() else {
            return false;
        };
        if !path_segment_has_assoc_binding(last, self.assoc_name) {
            return false;
        }
        let segments = path
            .segments
            ...
            .map(|segment| segment.ident.to_string())
            .collect::<Vec<_>>();
        self.resolve_trait_segments(&segments)
            .is_some_and(|(package, path)| {
                package == self.target_package && path == self.target_trait_path
            })
    }

    fn type_references_target_trait(&self, ty: &Type) -> bool {
        match ty {
            Type::Path(type_path) if type_path.path.is_ident("Self") => self.self_trait_is_target(),
            Type::Path(type_path) => {
                let segments = type_path
                    .path
                    .segments
            ...
            Type::Reference(reference) => self.type_references_target_trait(&reference.elem),
            Type::Group(group) => self.type_references_target_trait(&group.elem),
            Type::Paren(paren) => self.type_references_target_trait(&paren.elem),
            _ => false,
        }
    }

    fn self_trait_is_target(&self) -> bool {
        self.self_trait.is_some_and(|(package, path)| {
            package == self.target_package && path == self.target_trait_path
        })
    }

    fn resolve_trait_segments(&self, segments: &[String]) -> Option<(String, Vec<String>)> {
        let aliased = apply_alias(segments.to_vec(), self.aliases);
        let (package, path) = resolve_macro_invocation_segments(
            self.project,
            self.package,
...
impl Visit<'_> for TraitAssocPathReferenceVisitor<'_> {
    fn visit_expr_path(&mut self, node: &syn::ExprPath) {
        self.found |= self.path_references_target(&node.path, node.qself.as_ref())
            || self.path_has_target_assoc_binding(&node.path);
        if !self.found {
            visit::visit_expr_path(self, node);
        }
    }
    ...
    fn visit_path(&mut self, node: &syn::Path) {
        self.found |=
            self.path_references_target(node, None) || self.path_has_target_assoc_binding(node);
        if !self.found {
            visit::visit_path(self, node);
        }
    }
...
fn path_segment_has_assoc_binding(segment: &syn::PathSegment, assoc_name: &str) -> bool {
    ...
    arguments.args.iter().any(|argument| match argument {
        GenericArgument::AssocType(assoc_type) => assoc_type.ident == assoc_name,
        GenericArgument::Constraint(constraint) => constraint.ident == assoc_name,
        _ => false,
    })
}
...
fn known_macro_dependency_package_mentions(
    ...
    match code_name {
        "serde" | "serde_derive" => {
            macro_dependency_package_mentions_ident(
                project,
                reduced,
                render_plan,
                package,
        ...
        "thiserror" => {
            macro_dependency_package_mentions_ident(project, reduced, render_plan, package, "Error")
                && macro_dependency_package_mentions_ident(
                    project,
                    reduced,
                    render_plan,
        ...
        "strum" => macro_dependency_package_mentions_ident(
            project,
            reduced,
            render_plan,
            package,
        ...
        "bitflags" => macro_dependency_package_mentions_ident(
            project,
            reduced,
            render_plan,
            package,
        ...
        "error_support" => error_support_macro_idents().iter().any(|ident| {
            macro_dependency_package_mentions_ident(project, reduced, render_plan, package, ident)
        }),
        "error_support_macros" => macro_dependency_package_mentions_ident(
            project,
            reduced,
            render_plan,
            package,
        ...
        "lazy_static" => macro_dependency_package_mentions_ident(
            project,
            reduced,
            render_plan,
            package,
...
fn macro_dependency_package_mentions_ident(
    project: &Project,
    reduced: &ReducedProject,
    render_plan: Option<&RenderPlan>,
    package: &str,
    ...
    if let Some(render_plan) = render_plan {
        return render_plan.package_mentions_ident(package, ident);
    }
...
fn known_macro_dependency_target_should_remain(
    ...
    match dependency_code_name(first).as_str() {
        ...
        "serde" | "serde_derive" if matches!(leaf.as_str(), "Deserialize" | "Serialize") => {
            macro_dependency_package_mentions_ident(project, reduced, render_plan, package, leaf)
        }
        ...
        "error_support" => false,
        code_name => known_macro_dependency_package_mentions(
            project,
            reduced,
            render_plan,
            package,
            ...
            Item::Impl(item_impl) => {
                ...
                for impl_item in &item_impl.items {
                    if let ImplItem::Fn(method) = impl_item {
                        ...
                        let assoc_fn_surface_should_render = render_plan
...
fn trait_item_should_remain_for_type_surface(
    ...
    match item {
        ...
        TraitItem::Const(item) => {
            ...
            trait_assoc_item_should_remain_for_references(
                project,
                reduced,
                package,
                module_path,
        ...
        for module_path in project_module_paths(project, package) {
            ...
            for item in items {
                ...
                for impl_item in &item_impl.items {
                    ...
                    calls.extend(
                        associated_function_calls_on_types(method)
                            .into_iter()
                            .map(|(type_name, function)| (package.clone(), type_name, function)),
                    );
                }
            }
        }
...
fn associated_function_calls_on_types(item: &syn::ImplItemFn) -> BTreeSet<(String, String)> {
    let mut visitor = AssociatedFunctionCallsOnTypesVisitor::default();
    visitor.visit_impl_item_fn(item);
    visitor.calls
}
...
#[derive(Default)]
struct AssociatedFunctionCallsOnTypesVisitor {
    calls: BTreeSet<(String, String)>,
}

impl Visit<'_> for AssociatedFunctionCallsOnTypesVisitor {
    fn visit_expr_call(&mut self, node: &syn::ExprCall) {
        if let syn::Expr::Path(path) = node.func.as_ref() {
            let segments = path.path.segments.iter().collect::<Vec<_>>();
            if let Some(function) = segments.last().filter(|_| segments.len() > 1) {
                for type_segment in &segments[..segments.len() - 1] {
                    self.calls
                        .insert((type_segment.ident.to_string(), function.ident.to_string()));
                }
            }
        }
        visit::visit_expr_call(self, node);
...
fn impl_item_should_render_for_field_scan(
    ...
    if root_macro_impl_surface_should_render {
...
fn use_target_should_drop(
    ...
    if known_macro_dependency_target_should_remain(
        project,
        reduced,
        Some(render_plan),
        package,
        ...
        is_derive_only_external_trait_import(leaf) || is_thiserror_error_derive_import(target, leaf)
    }) {
        return false;
    }
        ...
        .is_some_and(|first| use_name_is_pruned_local_dependency(project, reduced, package, first))
    {
        ...
        return !known_macro_dependency_target_should_remain(
            project,
            reduced,
            Some(render_plan),
            package,
...
fn external_trait_import_should_remain(
    ...
    if leaf.ends_with("Ext") {
        ...
        if known_associated_functions.is_some_and(|functions| {
        ...
        let known_methods = known_trait_method_idents(target, leaf);
        let methods = if let Some(methods) = known_methods {
            methods.to_vec()
        } else {
        ...
        return target
            .first()
            .is_some_and(|first| matches!(first.as_str(), "std" | "core" | "alloc"))
            && known_associated_functions.is_none()
            && known_methods.is_none();
    }
