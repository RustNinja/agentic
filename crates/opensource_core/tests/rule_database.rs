...
#[test]
fn retains_helpers_referenced_by_out_dir_generated_expression_source() {
    let workspace = temp_path("rule-out-dir-expr-source-workspace");
    let output = temp_path("rule-out-dir-expr-source-output");
    let target_dir = temp_path("rule-out-dir-expr-source-target");
    write_out_dir_expression_source_rule_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("OUT_DIR expression include rule should reduce");
    ...
    let out_dir_include = report
        ...
        .expect("OUT_DIR expression include should remain production-blocking");
    assert!(out_dir_include.details.iter().any(|detail| {
        detail
            .blocked_idents
            .iter()
            .any(|ident| ident == "out_dir_generated_helper")
    }));

    let lib = read(output.join("out_dir_expr_source_rule/src/lib.rs"));
    assert!(lib.contains("include!(concat!(env!(\"OUT_DIR\"), \"/generated_expr.rs\"))"));
    assert!(lib.contains("out_dir_generated_helper"), "{lib}");
    assert!(!lib.contains("dead_out_dir_helper"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}
...
#[test]
fn retains_helpers_referenced_by_out_dir_format_generated_expression_source() {
    let workspace = temp_path("rule-out-dir-format-expr-source-workspace");
    let output = temp_path("rule-out-dir-format-expr-source-output");
    let target_dir = temp_path("rule-out-dir-format-expr-source-target");
    write_out_dir_format_expression_source_rule_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("OUT_DIR format expression include rule should reduce");
    ...
    let out_dir_include = report
        ...
        .iter()
        .find(|hazard| hazard.code == "out_dir_source_include_macros" && hazard.severity == "error")
        .expect("OUT_DIR format expression include should remain production-blocking");
    assert!(out_dir_include.details.iter().any(|detail| {
        detail
            .blocked_idents
            .iter()
            .any(|ident| ident == "out_dir_format_generated_helper")
    }));

    let lib = read(output.join("out_dir_format_expr_source_rule/src/lib.rs"));
    assert!(lib.contains("include!(concat!(env!(\"OUT_DIR\"), \"/generated_format_expr.rs\"))"));
    assert!(lib.contains("out_dir_format_generated_helper"), "{lib}");
    assert!(!lib.contains("dead_out_dir_format_helper"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}

#[test]
fn retains_helpers_referenced_by_out_dir_string_generated_item_source() {
    let workspace = temp_path("rule-out-dir-string-item-source-workspace");
    let output = temp_path("rule-out-dir-string-item-source-output");
    let target_dir = temp_path("rule-out-dir-string-item-source-target");
    write_out_dir_string_item_source_rule_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("OUT_DIR string item include rule should reduce");
    ...
    let out_dir_include = report
        ...
        .iter()
        .find(|hazard| hazard.code == "out_dir_source_include_macros" && hazard.severity == "error")
        .expect("OUT_DIR string item include should remain production-blocking");
    assert!(out_dir_include.details.iter().any(|detail| {
        detail
            .blocked_idents
            .iter()
            .any(|ident| ident == "out_dir_string_item_helper")
    }));

    let lib = read(output.join("out_dir_string_item_source_rule/src/lib.rs"));
    assert!(lib.contains("include!(concat!(env!(\"OUT_DIR\"), \"/generated_string_items.rs\"))"));
    assert!(lib.contains("out_dir_string_item_helper"), "{lib}");
    assert!(!lib.contains("dead_out_dir_string_item_helper"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}

#[test]
fn retains_helpers_referenced_by_out_dir_quote_generated_expression_source() {
    let workspace = temp_path("rule-out-dir-quote-expr-source-workspace");
    let output = temp_path("rule-out-dir-quote-expr-source-output");
    let target_dir = temp_path("rule-out-dir-quote-expr-source-target");
    write_out_dir_quote_expression_source_rule_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("OUT_DIR quote expression include rule should reduce");
    ...
    let out_dir_include = report
        ...
        .iter()
        .find(|hazard| hazard.code == "out_dir_source_include_macros" && hazard.severity == "error")
        .expect("OUT_DIR quote expression include should remain production-blocking");
    assert!(out_dir_include.details.iter().any(|detail| {
        detail
            .blocked_idents
            .iter()
            .any(|ident| ident == "out_dir_quote_generated_helper")
    }));

    let lib = read(output.join("out_dir_quote_expr_source_rule/src/lib.rs"));
    assert!(lib.contains("include!(concat!(env!(\"OUT_DIR\"), \"/generated_quote_expr.rs\"))"));
    assert!(lib.contains("out_dir_quote_generated_helper"), "{lib}");
    assert!(!lib.contains("dead_out_dir_quote_helper"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}

#[test]
fn retains_helpers_referenced_by_out_dir_quote_generated_item_source_with_attrs() {
    let workspace = temp_path("rule-out-dir-quote-attr-source-workspace");
    let output = temp_path("rule-out-dir-quote-attr-source-output");
    let target_dir = temp_path("rule-out-dir-quote-attr-source-target");
    write_out_dir_quote_attribute_source_rule_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("OUT_DIR quote item include rule should reduce");
    ...
    let out_dir_include = report
        ...
        .iter()
        .find(|hazard| hazard.code == "out_dir_source_include_macros" && hazard.severity == "error")
        .expect("OUT_DIR quote item include should remain production-blocking");
    assert!(out_dir_include.details.iter().any(|detail| {
        detail
            .blocked_idents
            .iter()
            .any(|ident| ident == "out_dir_quote_attr_helper")
    }));

    let lib = read(output.join("out_dir_quote_attr_source_rule/src/lib.rs"));
    assert!(lib.contains("include!(concat!(env!(\"OUT_DIR\"), \"/generated_quote_items.rs\"))"));
    assert!(lib.contains("out_dir_quote_attr_helper"), "{lib}");
    assert!(!lib.contains("dead_out_dir_quote_attr_helper"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}
...
#[test]
fn prunes_inherent_associated_const_when_only_name_is_mentioned() {
    let workspace = temp_path("rule-inherent-associated-const-shadow-workspace");
    let output = temp_path("rule-inherent-associated-const-shadow-output");
    let target_dir = temp_path("rule-inherent-associated-const-shadow-target");
    write_inherent_associated_const_shadow_name_fixture(&workspace);

    let report = generate(GenerateOptions {
        workspace_root: workspace,
        output_root: output.clone(),
    })
    .expect("shadow-name inherent associated const rule should reduce");

    assert_no_error_hazards(&report.production.hazards);
    let lib = read(output.join("inherent_assoc_const_shadow_rule/src/lib.rs"));
    assert!(lib.contains("pub const live: u32 = 7"), "{lib}");
    assert!(lib.contains("let shadowed = 1"), "{lib}");
    assert!(!lib.contains("pub const shadowed: u32 = 99"), "{lib}");
    assert_cargo_check(&output, &target_dir, &lib);
}
...
fn write_out_dir_expression_source_rule_fixture(root: &Path) {
    write_workspace(
        root,
        "out_dir_expr_source_rule",
        r#"use opensourced::opensourced;

#[opensourced]
pub fn selected() -> u32 {
    include!(concat!(env!("OUT_DIR"), "/generated_expr.rs"))
}
...
"#,
    ...
    let manifest = root.join("out_dir_expr_source_rule/Cargo.toml");
    let manifest_text = read(&manifest);
    fs::write(
        &manifest,
        manifest_text.replace(
    ...
    .expect("manifest should be writable");
    write(
        root.join("out_dir_expr_source_rule/build.rs"),
        r#"use std::{env, fs, path::PathBuf};
...
fn write_out_dir_format_expression_source_rule_fixture(root: &Path) {
    write_workspace(
        root,
        "out_dir_format_expr_source_rule",
        r#"use opensourced::opensourced;

#[opensourced]
pub fn selected() -> u32 {
    include!(concat!(env!("OUT_DIR"), "/generated_format_expr.rs"))
}

pub fn out_dir_format_generated_helper() -> u32 {
    42
}

pub fn dead_out_dir_format_helper() -> u32 {
    99
}
"#,
    );
    let manifest = root.join("out_dir_format_expr_source_rule/Cargo.toml");
    let manifest_text = read(&manifest);
    fs::write(
        &manifest,
        manifest_text.replace(
    ...
    .expect("manifest should be writable");
    write(
        root.join("out_dir_format_expr_source_rule/build.rs"),
        r#"use std::{env, fs::File, io::Write, path::PathBuf};

fn main() {
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR should be set"));
    let mut file = File::create(out_dir.join("generated_format_expr.rs"))
        .expect("generated format expression source should be created");
    writeln!(file, "{}{}", "out_dir_format_generated_helper", "()")
        .expect("generated format expression source should be writable");
}
"#,
    );
}

fn write_out_dir_string_item_source_rule_fixture(root: &Path) {
    write_workspace(
        root,
        "out_dir_string_item_source_rule",
        r#"use opensourced::opensourced;

include!(concat!(env!("OUT_DIR"), "/generated_string_items.rs"));

#[opensourced]
pub fn selected() -> u32 {
    generated_string_entry()
}

pub fn out_dir_string_item_helper() -> u32 {
    42
}

pub fn dead_out_dir_string_item_helper() -> u32 {
    99
}
"#,
    );
    let manifest = root.join("out_dir_string_item_source_rule/Cargo.toml");
    let manifest_text = read(&manifest);
    fs::write(
        &manifest,
        manifest_text.replace(
    ...
    .expect("manifest should be writable");
    write(
        root.join("out_dir_string_item_source_rule/build.rs"),
        r#"use std::{env, fs, path::PathBuf};

fn main() {
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR should be set"));
    fs::write(
        out_dir.join("generated_string_items.rs"),
        "fn generated_string_entry() -> u32 { out_dir_string_item_helper() }\n",
    )
        .expect("generated string item source should be writable");
}
"#,
    );
}

fn write_out_dir_quote_expression_source_rule_fixture(root: &Path) {
    write_workspace(
        root,
        "out_dir_quote_expr_source_rule",
        r#"use opensourced::opensourced;

#[opensourced]
pub fn selected() -> u32 {
    include!(concat!(env!("OUT_DIR"), "/generated_quote_expr.rs"))
}

pub fn out_dir_quote_generated_helper() -> u32 {
    42
}

pub fn dead_out_dir_quote_helper() -> u32 {
    99
}
"#,
    );
    let manifest = root.join("out_dir_quote_expr_source_rule/Cargo.toml");
    let manifest_text = read(&manifest);
    fs::write(
        &manifest,
        manifest_text.replace(
    ...
    .expect("manifest should be writable");
    write(
        root.join("out_dir_quote_expr_source_rule/build.rs"),
        r#"use std::{env, fs, path::PathBuf};
...
fn main() {
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR should be set"));
    fs::write(
        out_dir.join("generated_quote_expr.rs"),
        quote!(out_dir_quote_generated_helper()),
    )
        .expect("generated quote expression source should be writable");
}
"#,
    );
}

fn write_out_dir_quote_attribute_source_rule_fixture(root: &Path) {
    write_workspace(
        root,
        "out_dir_quote_attr_source_rule",
        r#"use opensourced::opensourced;

include!(concat!(env!("OUT_DIR"), "/generated_quote_items.rs"));

#[opensourced]
pub fn selected() -> u32 {
    generated_quote_entry()
}

pub fn out_dir_quote_attr_helper() -> u32 {
    42
}

pub fn dead_out_dir_quote_attr_helper() -> u32 {
    99
}
"#,
    );
    let manifest = root.join("out_dir_quote_attr_source_rule/Cargo.toml");
    let manifest_text = read(&manifest);
    fs::write(
        &manifest,
        manifest_text.replace(
    ...
    .expect("manifest should be writable");
    write(
        root.join("out_dir_quote_attr_source_rule/build.rs"),
        r#"use std::{env, fs, path::PathBuf};
...
fn main() {
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR should be set"));
    fs::write(
        out_dir.join("generated_quote_items.rs"),
        quote!(
            #[allow(dead_code)]
            fn generated_quote_entry() -> u32 {
                out_dir_quote_attr_helper()
            }
        ),
    )
        .expect("generated quote item source should be writable");
}
"#,
    );
}
...
fn write_inherent_associated_const_shadow_name_fixture(root: &Path) {
    write_workspace(
        root,
        "inherent_assoc_const_shadow_rule",
        r#"use opensourced::opensourced;
